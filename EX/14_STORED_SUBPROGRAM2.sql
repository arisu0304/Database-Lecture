...
-- ST_SC_GRADE 테이블에 학생 과목 기말고사 성적 등급까지 저장하는
-- 프로시저 P_ST_SC_GRADE 생성
...
-- 1-3. Stored Function : 오라클 사용자 정의 함수. 프로시저와 동일한 역할을
-- 하지만 프로시저와 다르게 리턴 값이 존재해서 EXEC 명령어로 단독 실행도 되고
-- 일반 쿼리문에서 호출도 가능하다.
...
BEGIN
    ...
    -- 위에서 지정한 함수 결과의 데이터 타입의 값을 RETURN 구문을 사용해서 리턴
...
-- 평점을 파라미터로 받아서 4.5 만점인 평점으로 환산하는 F_CONVERT_AVR
...
-- 날짜를 파라미터로 받아서 10년두 날짜를 리턴하는 함수 F_PLUS_10YEARS
...
-- 두 날짜를 파라미터로 받아서 두 날짜의 차이를 일자로 리턴하는 F_DIFF_DATES
...
TO_DATE('2010/10/10 12:15:30', 'YYYY/MM/DD HH:MI:SS')) as "차이날짜"
FROM DUAL;
...
-- 1-4. TRIGGER : 특정 테이블의 데이터 변경이 있을 때 지정한 PL/SQL 구문을 실행
-- 하는 SUB PROGRAM
...
BEGIN 
    IF :NEW.SAL < 3000 THEN 
        ...
        IF INSERTING THEN
            ...
            (-20001, '최저 시급보다 낮은 급여는 추가할 수 없습니다.');
        -- 데이터가 변경될 때
        ELSIF UPDATING THEN
            RAISE_APPLICATION_ERROR
            (-20002, '최저 시급보다 낮은 급여로 데이터를 수정할 수 없습니다.');
        ELSE
            RAISE_APPLICATION_ERROR
            (-20003, '최저 시급보다 낮음');
        END IF;
...
-- AFTER TRIGGER
-- SCORE 테이블의 데이터가 추가되거나 변경됐을 때 ST_SC_GRADE 테이블에
-- 데이터를 추가하거나 변경하는 트리거
