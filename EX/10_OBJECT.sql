-- 1. OBJECT(객체) : 데이터베이스에 데이터 보관 및 관리를 용이하게 하기 위한
-- 저장공간 또는 기능을 제공하는 역할
-- 테이블, 인덱스, 유저, 시퀀스, 시노님, ... 등 다양한 객체가 존재한다.

-- 1-1. 데이터 사전 : 사용자가 직접 생성한 테이블이 아닌 데이터 베이스의 성능과 관련된
-- 테이블로 데이터베이스 서버를 구축할 때 자동으로 생성되는 테이블들.
-- 직접 조작은 불가능하고 제공되는 뷰테이블로 SELECT 기능만 이용가능

-- C##STUDY 계정이 사용할 수 이쓴 데이터 사전 조회
SELECT *
FROM DICT;
...
-- C##STUDY는 현재 DBA 권한이 있기 때문에 관리자 계정으로 접근할 수 있는 모든 테이블을
-- 사용할 수 있다.

SELECT *
	FROM ALL_TABLES;

-- 관리자 (SYS) 계정이 소유하고 있는 시스템 관련 정보들을 담고 있는 테이블 목록 조회
...
-- DBA 권한이 있는 계정만 사용가능
-- ALTER SYSTEM KILL SESSION 'SID, SIREAL#' IMMEDIATE;
...
-- ALTER SYSTEM KILL SESSION '134, 22286' IMMEDIATE;	 
...
-- 데이터가 천만건 이상되는 테이블에 인덱스를 생성해야 인덱스의 효율이 증가	  
-- 인덱스는 고유인덱스와 비고유인덱스로 나눔
-- 고유인덱스는 인덱스로 지정된 컬럼의 데이터가 중복되면 안 되는 인덱스(PK, UK)
-- 비고유인덱스는 인덱스로 지정된 컬럼의 데이터가 중복을 허용하는 인덱스(직접 인덱스를 생성)
...
-- STUDENT 테이블에 4.5 만점으로 환산된 평점을 인덱스로 설정
...
-- 			   원천 테이블의 데이터가 변경되면 VIEW 테이블의 데이터도 자동으로 변경
--             단순 VIEW와 복합 VIEW로 나뉘게 되고
-- 단순 VIEW : 원천 테이블이 하나인 VIEW 테이블.
--   		 DML(INSERT, UPDATE, DELETE) 구문이 사용가능
-- 복합 VIEW : 여러개의 원천테이블을 조인해서 생성한 VIEW 테이블.
--           DML(INSERT, UPDATE, DELETE) 구문이 사용불가
-- 주로 통계데이터나 원천테이블의 데이터가 수시로 변경될 때 VIEW를 생성해서 데이터를 저장 후 사용
-- 과목별 학과별 기말고사 성적의 평균 점수를 저장하는 VIEW 생성
...
-- 1학년 학생의 정보만 모아놓은 VIEW기 때문에 다른 학년 학생의 데이터를 추가해도 
-- VIEW에서는 조회되지 않는다.
INSERT INTO ST_JUNIOR
VALUES (
	'999997',
...
-- 위치를 잘 지정해야하고 ROWNUM를 조회하는 SELECT 구문에서는 
-- <, <= 로만 비교가 가능하다.
-- ROWNUM를 조회하는 SELECT 구문을 서브쿼리로 묶어서 사용하면
-- > ,>= 로도 비교가능
SELECT B.*
   FROM(
 	SELECT ROWNUM AS RN
...
-- 1-4. SEQUENCE : 유일한 값을 만들어 제공해주는 객체. 
-- 주로 PK, UK와 함께 사용
...
VALUES(EMP_CO_ENO_SEQ1.NEXTVAL, '고기천', '개발', 0 , SYSDATE
		 , 3000, 100, 0);
COMMIT;
...
INSERT INTO EMP_COPY1 
VALUES(EMP_CO_ENO_SEQ1.NEXTVAL, '홍길동', '개발', 0 , SYSDATE
		 , 3300, 100, 0);
COMMIT;
...
INSERT INTO EMP_COPY1 
VALUES(EMP_CO_ENO_SEQ1.CURRVAL, '장길산', '개발', 0 , SYSDATE
		 , 3500, 100, 0);
COMMIT;
...
-- CURVALL은 주로 현재 어디까지 값이 할당됐는지 확인할 때 사용

SELECT DEPT_CO_DNO_SEQ.CURRVAL
	FROM DUAL;

-- NEXTVAL은 새로운 값을 할당받아서 값을 저장할 떄 주로 사용
-- NEXVAL이 호출되면 시퀀스는 무조건 새로운 값을 가리킨다.

SELECT DEPT_CO_DNO_SEQ.NEXTVAL
	FROM DUAL;

-- 시퀀스의 수정
-- 시퀀스는 모든 옵션을 수정할 수 있지만 시작 값을 수정하지 못한다.
...
-- BOARD_MAIN에 존재하는 테이블들을 SYNONYM을 통해서 BOARD_APP에서
-- 사용할 수 있는 동의어를 생성해서 개발자들은
-- BOARD_APP 계정을 통해서 DB에 접근할 수 있도록 한다.
-- (SELECT, INSERT, DELETE, UPDATE 권한만 줘서 사용)
-- SYNONYM 삭제
