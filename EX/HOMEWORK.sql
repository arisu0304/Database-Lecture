...
--1) 과목번호, 과목이름, 과목별 평균 기말고사 성적을 갖는 테이블 배열를 만들고
--   기본 LOOP문을 이용해서 모든 과목의 과목번호, 과목이름, 과목별 평균 기말고사 성적을 출력하세요.
...
DECLARE
	...
	TYPE COURSE_REC_ARR IS TABLE OF COURSE_REC 
	INDEX BY PLS_INTEGER;
	
	COURSERECARR COURSE_REC_ARR;

	IDX NUMBER := 1;

BEGIN
	FOR COURSE_AVG_ROW IN(
		SELECT C.CNO
			 , C.CNAME
			 , AVG(SC.RESULT) AS AVG_RES
		FROM COURSE C
	...
	COURSERECARR(IDX).CNO := COURSE_AVG_ROW.CNO;
	COURSERECARR(IDX).CNAME := COURSE_AVG_ROW.CNAME;
	COURSERECARR(IDX).CNO := COURSE_AVG_ROW.CNO;	
	
	DBMS_OUTPUT.PUT_LINE(COURSERECARR(IDX).CNO);
	DBMS_OUTPUT.PUT_LINE(COURSERECARR(IDX).CNAME);
	DBMS_OUTPUT.PUT_LINE(COURSERECARR(IDX).AVG_RES);
	IDX := IDX + 1;
