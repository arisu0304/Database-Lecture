...
-- 데이터를 저장, 수정, 삭제하는 언어로
-- INSERT INTO, UPDATE SET, DELETE FROM이 존재
-- DML은 트랜잭션을 완료하기 위해 항상 COMMIT이나 ROLLBACK을 동반
-- 1-1. INSERT INTO : 테이블에 데이터를 저장하는 명령어
-- 일부 컬럼에 대한 데이터를 저장

INSERT INTO EMP(ENO, ENAME, JOB, HDATE)
	 VALUES('9997', '장길산', '설계', SYSDATE);
COMMIT;
...
INSERT INTO EMP(ENO, ENAME, JOB, HDATE)
	 VALUES('9996', '임꺽정', '개발', SYSDATE);
INSERT INTO EMP(ENO, ENAME, JOB, HDATE)
	 VALUES('9995', '홍길동', '회계', SYSDATE);
INSERT INTO EMP(ENO, ENAME, JOB, HDATE)
	 VALUES('9994', '조병조', '지원', SYSDATE);
INSERT INTO EMP(ENO, ENAME, JOB, HDATE)
	 VALUES('9993', '정도전', '경영', SYSDATE);
-- COMMIT이 발생한 작업에 대해서는 ROLLBACK으로 취소할 수 없다.	
ROLLBACK;

-- INSERT INTO 시 지정한 컬럼 개수, 타입 지켜야함

-- 에러
/*
INSERT INTO EMP(ENO, ENAME, JOB, HDATE) 
	VALUES('9993', '정도전', '경영');
INSERT INTO EMP(SAL) VALUES ('aaaaa');
*/

-- VARCHAR 타입의 숫자값이 NUMBER 타입을 형변환이 일어나면서 저장
INSERT INTO EMP(SAL) VALUES('12332');

-- 모든 컬럼에 데이터를 저장할 때는 컬럼지정을 생략해도 된다.
INSERT INTO EMP(ENO, ENAME, JOB, MGR, HDATE, SAL, COMM, DNO)
	VALUES('9996', '임꺽정', '회계', '0001', SYSDATE,
			4000, 300, '10');

INSERT INTO EMP VALUES('9995', '조병조', '경영', '0201', 
		SYSDATE, 3700, 280, '20');
...
-- SELECT 구문을 이용해서 다량의 데이터 저장시 특정 컬럼 데이터만 저장 가능
-- DNO이 10인 사원의 사원번호, 사원이름만 EMP 테이블에서 조회해서 
-- EMP_DNO30 테이블에 저장
...
CREATE TABLE COURSE_PROFESSOR(
	...
	ST_NUM NUMBER(1,0),
	PNUM VARCHAR2(8),
	PROFFESSOR_NAME VARCHAR2(20)
);

-- 모든 과목의 과목번호, 과목이름, 학점, 담당하는 교수의 교수번호, 교수이름을
-- COURSE_PROFFESSOR에 저장
...
-- WHERE 절로 특정 데이터만 수정
-- EMP_DNO30 테이블에서 ENO 2007인 사람의 급여를 3100으로 수정
...
-- EMP_DNO30 테이블에서 ENO이 3004인 사원의 업무를 경영으로
-- MGR은 2003으로 HDATE는 오늘날짜로 SAL은 5000으로
-- 보너스는 500으로 수정
UPDATE EMP_DNO30 
	SET 
		JOB = '경영',
...
-- EMP_DNO30 테이블에서 DNO이 20, 30인 부서를 60 부서로 통합하고
...
-- 서브쿼리를 이용한 데이터 수정
-- SET 절에 서브쿼리의 결과로 나오는 컬럼 수만큼 ()묶은 컬럼을 지정하고
--  = 다음에 서브쿼리를 작성한다.
-- ()로 묶인 컬럼 개수와 타입이 서브쿼리의 결과의 데이터의 컬럼, 타입과 일치해야한다.
...
--STUDENT_COPY1 테이블에서 기말고사 성적의 평균이 60점 이하인 학생을 삭제
