-- 1. CONSTRAINT (제약조건) : 제약조건은 데이터의 정확성과 무결성을 보장해주는
-- 			데이터를 저장하는 규칙
--			제약조건에는 PK, FK, UK, NOT NULL, DEFAULT, CHECK 등이 존재
-- 1-1. PK(PRIMARY KEY) : 
--		 	테이블에 식별자 역할을 해주는 컬럼을 지정
--			PK로 지정된 컬럼의 데이터는 유일해야하며(UK), 색인 역할도 하며(INDEX)
...
-- PK로 설정된 컬럼의 데이터는 유일한 값(UK)을 가져야 하기 때문에
-- 중복된 데이터를 저장할 수 없다.
INSERT INTO EMP_PK1
...
-- 다중 컬럼으로 PK를 지정하게 되면 PK로 지정된 컬럼들을 데이터 셋으로 묶어서
-- PK로 인식한다. PK로 지정된 컬럼들의 데이터 모두 중복되어야 중복으로 인식
...
-- PK로 지정된 모든 컬럼에 NULL 값은 허용되지 않는다. (하나라도)
/*
INSERT INTO PROFESSOR_PK1
...
CREATE TABLE BOARD_FILE(
	...
	CONSTRAINT PK_BOARD_FILE_FILE_NO PRIMARY KEY
	(BOARD_FILE_NO)
);
...
CREATE TABLE BOARD_FILE_MULTIPK(
	...
	BOARD_FILE_NO NUMBER(10),
	BOARD_FILE_NAME VARCHAR2(1000),
	CONSTRAINT PK_BOARD_FILE_BOARD_FILE_NO PRIMARY KEY
	(BOARD_NO, BOARD_FILE_NO)
);
...
-- 기존 테이블에 PK 추가
-- 기존 테이블의 구조를 변경하는 작업이기 때문에
-- ALTER TABLE 구문을 사용한다.
-- PK로 지정될 컬럼의 데이터에 중복값이나 NULL값이 없어야한다.
...
-- 기본 테이블에 다중컬럼 PK 추가
ALTER TABLE SCORE
ADD CONSTRAINT PK_SCORE_SNO_CNO PRIMARY KEY
						(SNO, CNO);
...
-- 1-2. FOREIGN KEY(외래키, 참조키):
--   	다른 테이블의 PK나 UK로 설정된 컬럼을 참조하여 제약조건을 생성
--		참조한 컬럼에 존재하는 데이터만 저장할 수 있는 제약조건이 생성된다.
-- 		테이블 간의 관계를 맺어주는 제약조건
--		참조하는 테이블이 부모 테이블이 되고 참조해오는 테이블이 자식 테이블이
--		돼서 테이블 간의 종속관계를 형성한다.
--		부모테이블에 존재하는 데이터만 저장가능
-- 		관계에는 1:1 , 1:N, N:1 , N:N 등 다양한 관계가 존재
-- DEPT_PK1 테이블의 PK로 설정된 DNO을 참조한 FK를 갖는
-- EMP_FK1 테이블 생성
CREATE TABLE EMP_FK1(
	ENO NUMBER(4),
	ENAME VARCHAR2(20),
	-- FK로 지정될 컬럼의 데이터 타입은 참조하는 컬럼의 데이터 타입과 
	-- 일치해야 한다.
	-- DEPT_PK1 테이블에 DNO컬럼에 존재한 데이터들만
	-- DNO 컬럼에 저장할 수 있다.
...
-- 부모 테이블인 DEPT_PK1에 존재하지 않는 DNO 3은
-- 자식 테이블인 EMP_FK1의 DNO로 사용할 수 없다.
/*
INSERT INTO EMP_FK1
VALUES (3, '장길산',3);
*/
...
-- FK로 관계가 형성되면 부모테이블에서 데이터의 수정이나 삭제가 자유롭게 
-- 진행되지 않는다. 
-- 자식 테이블에서 사용하고 있는 부모테이블의 데이터가 없어야 수정이나 삭제가 가능
INSERT INTO DEPT_PK1 
VALUES (3, '회계', '대전', 3);
COMMIT;
...
-- 컬럼을 모두 지정하고 FK 생성
-- CASCADE 옵션 추가하여 생성, ORACLE에서는 DELETE CASCADE
-- 옵션만 추가 가능
-- CASCADE 옵션을 추가하려면 부모테이블 데이터의 수정이나 삭제가 일어날 경우
...
-- ON DELETE CASCADE 옵션에 의해서 부모테이블인 DEPT_PK1의
-- 데이터를 삭제하면 참조하고 있는 자식테이블인 EMP_FK2에서도
-- 데이터가 삭제된다.
DELETE FROM DEPT_PK1
	WHERE DNO = 1;
COMMIT;
...
-- COURSE의 CNO를 FK로 받아오면서 PK로 지정
ALTER TABLE COURSE ADD CONSTRAINT RK_SCORE_CNO 
	PRIMARY KEY(CNO);
...
-- FK면서 식별자 역할을 하기 때문에 데이터의 중복이 허용되지 않고
-- NULL 값도 저장할 수 없다.
...
-- FK는 여러번 지정할 수 있다.
-- STUDENT 테이블의 SNO를 참조하고, COURSE의 CNO를 참조해서
-- FK로 설정하고, SNO, CNO의 묶음을 PK로 지정하는 테이블
-- ST_SCORE_PK_FK를 생성하세요. (SNO, CNO 만 컬럼으로 생성)

ALTER TABLE STUDENT ADD CONSTRAINT PK_STUDENT_SNO 
PRIMARY KEY(SNO);

CREATE TABLE ST_SCORE_PK_FK(
	SNO VARCHAR2(8),
	CNO VARCHAR2(8),
	CONSTRAINT PK_ST_SCORE_PK_FK_SNO_CNO
		PRIMARY KEY(SNO, CNO),
	CONSTRAINT FK_ST_SCORE_PK_FK_SNO
		FOREIGN KEY(SNO) REFERENCES STUDENT(SNO),
	CONSTRAINT FK_ST_SCORE_PK_FK_CNO
		FOREIGN KEY(CNO) REFERENCES COURSE(CNO)
);

-- STUDENT 테이블에 SNO로 존재하는 데이터와 COURSE 테이블에 CNO로
-- 존재하는 데이터만 저장할 수 있고 데이터의 묶음이 중복이면 안 되고
-- NULL 값을 허용하지 않는 테이블
...
-- 1대 N관계, N대 1관계 : 부모 테이블에 존재하는 데이터가 자식테이블에
-- 					여러개 존재하는 구조
...
-- N대N 관계 (다대다 관계) : 두 개의 테이블의 데이터가 여러개가 존재하는
--					N대N관계가 매핑된 구조
-- 중간에 N대N으로 매핑된 테이블이 필요하다.
CREATE TABLE LECTURE(
	LECTURE_ID NUMBER(9),
	LECTURE_NAME VARCHAR2(30),
	CONSTRAINT PK_LECTURE_ID PRIMARY KEY(LECTURE_ID)
);
...
CREATE TABLE LECTURE_STUDENT(
	LECTURE_ID NUMBER(9),
	STUDENT_ID NUMBER(9),
	CONSTRAINT PK_LECTURE_STUDENT_IDS 
		PRIMARY KEY(LECTURE_ID, STUDENT_ID),
	CONSTRAINT FK_LECTURE_STUDENT_L_ID
		FOREIGN KEY(LECTURE_ID)
		REFERENCES LECTURE(LECTURE_ID),
	CONSTRAINT FK_LECTURE_STUDENT_S_ID
...
-- 	    UK는 중복값 불가만 가지고 있는 제약조건이다.
--		그래서 NULL 값이 저장된다.

CREATE TABLE EMP_UK(
	ENO NUMBER(4)
	CONSTRAINT PK_EMP_UK_ENO PRIMARY KEY,
	ENAME VARCHAR2(20)
	CONSTRAINT UK_EMP_UK_ENAME UNIQUE
);
...
-- UK로 지정된 컬럼에는 NULL값이 저장되고 NULL값은 중복도 허용
INSERT INTO EMP_UK
VALUES(3, NULL);
INSERT INTO EMP_UK
VALUES(4, NULL);
...
-- 			CHECK가 지정되어 있는 컬럼의 데이터는 CHECK에 지정된
-- 			조건에 부합하는 데이터만 저장할 수 있다.
CREATE TABLE EMP_CHK(
	ENO NUMBER(4) PRIMARY KEY,
	ENAME VARCHAR2(20) UNIQUE,
	SAL NUMBER(5,0) CHECK(SAL >= 3000),
	COMM NUMBER(5,0),
	CONSTRAINT CHK_EMP_CHK_COMM 
...
-- NOT NULL 제약조건이 지정된 칼럼의 데이터로 NULL 저장하면 에러가 발생
/*
...
-- 1-6. DEFAULT : 특정 칼럼의 데이터가 NULL로 오거나 들어오지 않았을 때를
--  		대비해서 기본값을 지정해놓는 제약조건
...
-- FNO(NUMBER(10), FK(FACTORY1의 FNO), NOT NULL)
-- 컬럼을 갖는 GOODS1
...
-- PNO(NUMBER(6, PK)), 
-- GNO(NUMBER(5), FK(GOODS1의 GNO), NOT NULL)
