-- 1. PL/SQL : 오라클에서 제공하는 절차적 언어를 만들기 위한 기능
--    (Procedual Language Extension to SQL)
-- 	   쿼리문에 변수를 선언하고 조건문이나 반복문을 사용할 수 있다.
-- PL/SQL에서는 결과를 출력하기 위해서 DBMS_OUTPUT.PUT_LINE
-- 함수를 이용한다.
...
DECLARE 
	...
	-- 선언부에서 변수의 값을 할당하지 않으면 실행부에서 할당해도 된다.
	NUM NUMBER;
BEGIN 
	NUM := 10;
...
DECLARE 
	...
	-- STUDENT_ROW 변수에는 SNO, SNAME, SEX, MAJOR, AVR
	-- 포함되어 있다.
...
BEGIN
	...
	IF PROFESSOR_ROW.ORDERS = '정교수' THEN
		DBMS_OUTPUT.PUT_LINE(
		PROFESSOR_ROW.PNAME || ' 교수는 정교수입니다.'
		);
	ELSIF PROFESSOR_ROW.ORDERS = '부교수' THEN
		DBMS_OUTPUT.PUT_LINE(
		PROFESSOR_ROW.PNAME || ' 교수는 부교수입니다.'
		);
	ELSE 
		DBMS_OUTPUT.PUT_LINE(
		PROFESSOR_ROW.PNAME || ' 교수는 시간강사입니다.'
		);
	END IF;
END;
...
BEGIN
	LOOP
		-- 조회하는 데이터의 행의 개수가 0개면 NO_DATA_FOUND 예외 발생
...
DECLARE
	...
	-- 위에 선언한 레코드 자료형으로 변수를 하나 선언해야 실행부에서
	-- 레코드를 사용할 수 있다.
	-- 레코드 자료형의 변수 선언
...
BEGIN
	LOOP
		SELECT DECODE(MOD(IDX, 2), 0 , '남' , '여')
				INTO STURECARR(IDX).SEX
				FROM DUAL;
...
-- STUDENT_RECORD 테이블에 SNO 20001~20009 번까지 학생을 저장하세요.
-- SNO, SNAME, SEX, SYEAR, MAJOR, AVR은 위의 내용 참고

DECLARE

	-- 위에서 선언한 레코드 타입의 연관배열선언
	TYPE STU_REC_ARR IS TABLE OF STUDENT%ROWTYPE
	INDEX BY PLS_INTEGER;
...
BEGIN
	LOOP
		SELECT DECODE(MOD(IDX, 2), 0 , '남' , '여')
				INTO STURECARR(IDX).SEX
				FROM DUAL;
		STURECARR(IDX).SNO := TO_CHAR(20000 + IDX);
		STURECARR(IDX).SNAME := '학생' || (20000 + IDX);
		STURECARR(IDX).SYEAR := MOD(IDX, 4) + 1;
		STURECARR(IDX).MAJOR := '컴공';
		STURECARR(IDX).AVR := MOD(IDX, 4) + 0.24;
...
DECLARE	
	...
	-- 변수의 형태는 일반 변수, 레코드, 참조타입 변수여도 상관없음
...
BEGIN
	...
	LOOP
		...
		-- 커서명%NOTFOUND : 더 이상 꺼내올 데이터가 없으면 TRUE,
	 	--					데이터가 있으면 FALSE
...
-- FOR LOOP에서는 CURSOR의 OPNE, FETCH, CLOSE가 자동으로 이뤄진다.
...
--  			커서를 오픈할 때 파라미터로 지정한 데이터 타이브이 값을 함께 넘겨준다.

DECLARE 
	-- 파라미터가 있는 커서의 선언
	-- 파라미터들은 조회 쿼리의 조건절(WHERE 절)에서 사용해서 유동적인 쿼리를
	-- 실행할 수 있게 해준다.
 	...
 	BEGIN
 		-- 커서를 오픈할 때 지정된 파라미터 데이터 타입의 값들을 전달한다.
	 	OPEN STU_CUR(1, 2.7);
 ...
 -- 파라미터로 전달된 부서번호와 부서번호가 같고 파라미터로 전달된 급여 이상되는 급여를
 -- 받는 사원 목록을 조회
 -- EMP 테이블의 모든 컬럼을 조회
 ...
 DECLARE 
	...
	-- 실행할 수 있게 해준다.
 	...
 	BEGIN
	 	OPEN EMP_CUR('30', 2000);
	 	
	 	LOOP 
		 	FETCH EMP_CUR INTO EMP_ROW;
	 	
		 	DBMS_OUTPUT.PUT_LINE
		 			('30 부서에서 급여가 2000이상인 사원목록');
	 		DBMS_OUTPUT.PUT_LINE(EMP_ROW.ENO);
	 		DBMS_OUTPUT.PUT_LINE(EMP_ROW.ENAME);
	 		DBMS_OUTPUT.PUT_LINE(EMP_ROW.SAL);
	 		DBMS_OUTPUT.PUT_LINE(EMP_ROW.DNO);
 ...
 -- 6-4. 묵시적 커서 : 쿼리를 실행하면 쿼리의 실행 결과를 오라클 내부에서 
 --				저장하고 있는데 오라클 내부에 저장된 쿼리의 결과를 확인
 --				할 수 있는 커서. 따로 커서를 선언하지 않고 사용할 수
 -- 			있는 커서. SQL%NOTFOUND, SQL%FOUND
 --				SQL%ROWCOUNT, SQL%ISOPEN 속성만 사용할 수 있다.
 ...
 BEGIN 
 	...
 	-- 묵시적 커서를 이용해서 위 커리에 영향받은 행의 개수를 가져올 수 있다.
 	DBMS_OUTPUT.PUT_LINE(SQL%ROWCOUNT);
 
 	COMMIT;
 END;

-- 7. 예외처리 : EXCEPTION 부에서 PL/SQL이 동작하다가 발생할 수 있는
--			예외에 대한 처리를 작성한다.
DECLARE
	VAL_SNO NUMBER;
BEGIN
	...
	SELECT SNO INTO VAL_SNO
	FROM STUDENT 
	WHERE SYEAR = 1;

	DBMS_OUTPUT.PUT_LINE('예외 발생 시 COMMIT 실행 안 됨');
	COMMIT;
EXCEPTION
	WHEN VALUE_ERROR THEN
		DBMS_OUTPUT.PUT_LINE('수치가 부적합합니다.');
		-- EXCEPTION 부에서는 대부분 PL/SQL에서 에러가 발생했을때
...
EXCEPTION
	...
	DBMS_OUTPUT.PUT_LINE('쿼리의 결과가 한 행이 아닙니다.');
		...
		-- ROLLBACK;을 사용한다.
		ROLLBACK;
	-- 지정된 예외 이름 말고 다른 예외들을 처리할 때는 OTHERS를 사용
	-- 자바에서 마지막 CATCH에 최상위 클래스인 EXCEPTION을 사용하는 것과
	-- 동일
	WHEN OTHERS THEN
